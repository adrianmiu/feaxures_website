        ...
        deps.domReady(function() {
            ...
            each(self._features, function(feature, featureName) {
                if (allowedEvents.indexOf(feature.attachEvent) !== -1) {
                    var evtName = feature.attachEvent, callback;
                    callback = function(evt, data) {
                          // if the current element is not a candidate for the feature, return asap
                          if (self.getFeatureOptionsForElement(featureName, evt.currentTarget) === false) {
                              return;
                          }
                          evt.stopPropagation();
                          evt.preventDefault();
                          ...
                          self.attach(featureName, [evt.currentTarget]);
                          ...
                          self.on('load:' + featureName, undelegate);
                          self.on('loadError:' + featureName, undelegate);

                          self.one('attach:' + featureName, function(e) {
                              // trigger the event again
                              deps.triggerEvent(evt.currentTarget, evtName);
